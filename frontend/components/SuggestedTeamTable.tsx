import { playerSupportLabel, type SuggestedPlayer } from "./suggestedTeam";
  ...
  return (
    <section className="data-card suggested-team-table-card" aria-labelledby="suggested-team-details-title">
      ...
      <div className="table-scroll"><table className="suggested-team-table">
        <thead><tr><th>Player</th><th>Pos</th><th>Team</th><th>Fixture</th><th>Price</th><th>Pred. pts</th><th>Expert support</th><th>Own.</th><th>FDR</th><th>Status</th></tr></thead>
        <tbody>{players.map((player, index) => <tr key={`${player.playerId}-${index}`}>
          ...
          <td>{value(player.predictedPoints)}</td><td>{playerSupportLabel(player)}{player.support && player.captain ? <small>Captain pick: {player.support.captainSupportCount} of {player.support.eligibleExpertCount} experts</small> : null}{player.support && player.viceCaptain ? <small>Vice-captain pick: {player.support.viceCaptainSupportCount} of {player.support.eligibleExpertCount} experts</small> : null}</td><td>{value(player.ownership, "%")}</td><td>{value(player.fixtureDifficulty)}</td>
