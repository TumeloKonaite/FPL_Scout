...
- **Fabricated XI when evidence is insufficient.** When `SuggestedTeam.constructionStatus === "insufficient_evidence"` (or when there is no legal 11-player lineup), `SuggestedTeamPitch` renders `SectionUnavailableState` with a friendly diagnostic — it does **not** synthesize players or expose internal failure codes. A complete bench is optional.
