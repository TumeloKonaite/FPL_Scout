...
class FplApiClient:
    ...
    def get_upcoming_gameweek(self) -> CurrentGameweek | None:
        ...
        request = Request(self.url, headers={"Accept": "application/json", "User-Agent": "FPL-Scout/1.0"})
        try:
            with urlopen(request, timeout=self.timeout_seconds) as response:  # noqa: S310 - fixed HTTPS service URL
                payload: Any = json.load(response)
        except (HTTPError, URLError, TimeoutError, json.JSONDecodeError, OSError) as exc:
            raise FplApiError("Could not retrieve the official FPL gameweek calendar") from exc

        events = payload.get("events") if isinstance(payload, dict) else None
        if not isinstance(events, list):
            raise FplApiError("The official FPL gameweek calendar response was invalid")

        event = next((item for item in events if isinstance(item, dict) and item.get("is_next") is True), None)
        if event is None:
